...
async def iniciar_turno(id_turno_m2: int, token_auth: str | None = None) -> dict:
    ...
    if settings.integraciones_mockeadas:
        logger.info("🧪 [MOCK M2] PATCH /appointments/%s/start", id_turno_m2)
    ...
    logger.info("📡 [M2] Iniciando turno %s en M2: %s", id_turno_m2, url)
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            data = resp.json()
            logger.info("✅ [M2] Turno %s iniciado exitosamente en M2: %s", id_turno_m2, data)
            return data
...
async def finalizar_turno(id_turno_m2: int, token_auth: str | None = None) -> dict:
    ...
    if settings.integraciones_mockeadas:
        logger.info("🧪 [MOCK M2] PATCH /appointments/%s/finish", id_turno_m2)
        return {
    ...
    logger.info("📡 [M2] Finalizando turno %s en M2: %s", id_turno_m2, url)
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            resp.raise_for_status()
            data = resp.json()
            logger.info("✅ [M2] Turno %s finalizado exitosamente en M2: %s", id_turno_m2, data)
            return data
        except httpx.HTTPStatusError as exc:
